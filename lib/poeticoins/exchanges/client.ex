defmodule Poeticoins.Exchanges.Client do
  ...
  defmacro defclient(options) do
    ...
    quote do
      ...
      def child_spec(opts) do
        {currrency_pairs, opts} = Keyword.pop(opts, :currency_pairs, available_currency_pairs())

        %{
          id: __MODULE__,
          start: {unquote(__MODULE__), :start_link, [__MODULE__, currrency_pairs, opts]}
        }
      end
  ...
  defp connect(client) do
    {:ok, conn} =
      :gun.open(server_host(client.module), server_port(client.module), %{protocols: [:http]})
